module.exports = function() {
	...
	function(energyAvail, name) {
		var moveParts = Math.min(Math.floor((energyAvail-(BODYPART_COST[WORK]+BODYPART_COST[MOVE]+(BODYPART_COST[CARRY]))/(BODYPART_COST[MOVE]+(BODYPART_COST[CARRY]*2))),6);
		...
		return this.spawnCreep(body, name, {memory: {role: 'supplier', moveParts: moveParts, totalParts: moveParts * 3 + 1}});
	}
};
